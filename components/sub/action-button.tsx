...
export const ActionButton = ({
  ...
  const inner = (innerClassName?: string) => (
    <span
      className={cn(
        "inline-flex h-full w-full items-center justify-center gap-2 rounded-lg bg-slate-950 px-7 text-sm font-medium backdrop-blur-3xl",
        innerClassName
      )}
    >
      {icon}
  ...
  if (!available) {
    return (
      <span
        ...
        className={cn(
          "relative inline-flex h-12 w-full select-none overflow-hidden rounded-lg p-[1px] opacity-50 saturate-50 md:w-60",
          "cursor-not-allowed",
          className
        )}
      >
  ...
  return (
    <Link
      ...
      className={cn(
        "group relative inline-flex h-12 w-full overflow-hidden rounded-lg p-[1px] transition focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/60 md:w-60",
        className
      )}
    >
